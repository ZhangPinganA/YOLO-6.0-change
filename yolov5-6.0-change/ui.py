...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "目标检测程序"))
    ...
    def button_image_open(self):
        ...
        showing = cv2.resize(im0, (640, 480))
    ...
    def show_video_frame(self):
        ...
        if img is not None:
            ...
            with torch.no_grad():
                ...
                for i, det in enumerate(pred):  # detections per image
                    if det is not None and len(det):
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            # if (self.names[int(cls)] == "chair" or self.names[int(cls)] == "person"):
                            label = '%s %.2f' % (self.names[int(cls)], conf)
                            name_list.append(self.names[int(cls)])
                            print(label)
                            plot_one_box(
            ...
            # self.out.write(showing)
            # showing = cv2.resize(showing, (640, 480))


            self.result = cv2.cvtColor(showing, cv2.COLOR_BGR2RGB)
