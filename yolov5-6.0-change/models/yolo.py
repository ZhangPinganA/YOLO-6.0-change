...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                BottleneckCSP, C3, C3TR, C3SPP, seC3,C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d,
                 C3x,CBRM,Shuffle_Block,GAMAttention,h_sigmoid,h_swish,SELayer,conv_bn_hswish,MobileNet_Block,
                 RepVGGBlock,GSConv,VoVGSCSP,CTR3,C3STR):
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost, C3x,VoVGSCSP,C3STR]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5s-asff-se.yaml', help='model.yaml')
